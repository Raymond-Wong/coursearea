...
def login(request):
  ...
  if len(request.session['admin']) > 0 and\
     ...
     (request.session['admin'][-1].time - now).total_seconds() < ADMIN_CONFIG['retry_freq'][2]:
  ...
  if coursearea.utils.md5(account) == coursearea.utils.md5(ADMIN_CONFIG['account']) and\
     coursearea.utils.md5(passwd) == coursearea.utils.md5(ADMIN_CONFIG['passwd']):
    request.session['admin'] = []
    request.session['admin'].append(Login_Record(ADMIN_STATUS.logined))
    return HttpResponse(Response(msg=u'登录成功').to_json(), content_type='application/json')
  # 如果账号或者密码错误，判断是否要上锁
  if len(filter(lambda x:x.status == ADMIN_STATUS.failed and (now - x.time).total_seconds() < ADMIN_CONFIG['retry_freq'][0], request.session['admin'])) > ADMIN_CONFIG['retry_freq'][1]:
    request.session['admin'].append(Login_Record(ADMIN_STATUS.locked))
    unlocked_time = request.session['admin'][-1].time + timedelta(seconds=ADMIN_CONFIG['retry_freq'][2])
    unlocked_time = unlocked_time.strftime('%Y-%m-%d %H:%M')
...
def get_course_detail(request):
  ...
  for c in course_obj.recommend_courses:
...
def set_series_detail(request):
  ...
  if series_id is not None:
    ...
    series = Series()
  series.name = coursearea.utils.parse_arg(request.POST, 'name', series.name, str, None)
...
def set_course_detail(request):
  ...
  # 设置课程基本信息
  info = coursearea.utils.parse_arg(request.POST, 'info', None, None, lambda x:(x is None or isinstance(json.loads(x), dict)))
  if info is not None:
    info_obj = course.info if course.info is not None else Course_Info()
    ...
    teacher_obj = info_obj.teacher if info_obj.teacher is not None else Teacher()
    ...
    info_obj.save()
  # 设置组
  group = coursearea.utils.parse_arg(request.POST, 'group', None, None, lambda x:(x is None or isinstance(json.loads(x), dict)))
  if group is not None:
    group_obj = course.group if course.group is not None else Group()
  ...
  if recommend_courses is not None:
    ...
    recommend_courses = filter(lambda x:Course.objects.filter(id=x).count() > 0, json.loads(recommend_courses))
  ...
  # 设置海报样式
  style = coursearea.utils.parse_arg(request.POST, 'style', None, None, lambda x:(x is None or isinstance(json.loads(x), dict)))
  if style is not None:
    style_obj = course.style if course.style is not None else Course_Style()
